...
const CharacterTile = ({variant, panda, selected, setSelectedPanda, setHoveredPanda, className}: Props) => {
    ...
    return (
        <>
            <div onMouseLeave={ setHoveredPanda ? () => setHoveredPanda([false, ""]) : () => {} } onMouseOver={ setHoveredPanda ? () => setHoveredPanda([true, cPanda?.name]) : () => {} } onClick={ setSelectedPanda ? () => setSelectedPanda(cPanda?.name) : () => {} } className={`group w-28 md:w-32 lg:w-40 xl:w-44 2xl:w-52 aspect-square cursor-pointer select-none ${className}`}>
